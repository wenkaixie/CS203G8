...
public class AdminService {
    ...
    */
    public void completeMatch(String tournamentId, String roundNum, int matchId,
            MatchResultUpdateRequest matchResultUpdateRequest) {
        CollectionReference tournamentsRef = firestore.collection("Tournaments");
        ...
        try {
            ...
            String user1 = (String) participants.get(0).get("authId");
            String user2 = (String) participants.get(1).get("authId");
    ...
    */
    public List<Tournament> getAdminTournaments(String adminId) {
        ...
        try {
            ...
            CollectionReference tournamentsRef = firestore.collection("Tournaments");
            ApiFuture<QuerySnapshot> tournamentsQuery = tournamentsRef.whereIn(FieldPath.documentId(), tournamentIDs)
                    .get();
    ...
    */
    ...
    public boolean validateAdmin(String adminID) throws ExecutionException, InterruptedException {
        DocumentReference adminRef = firestore.collection("Admins").document(adminID);
        DocumentSnapshot adminSnapshot = adminRef.get().get();
        boolean exists = adminSnapshot.exists();
        log.info("Validation result for admin ID {}: {}", adminID, exists);
        return exists;
    }

    public void addTournamentToAdmin(String authId, String generatedId)
        ...
        adminRef.update("tournamentCreated",
                FieldValue.arrayUnion(generatedId)).get();
        log.info("Tournament ID {} added to admin {}'s tournamentCreated field.",
                generatedId, authId);
    }
    
    public void removeTournamentFromAdmin(String adminID, String tournamentID)
