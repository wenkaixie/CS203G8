...
const AdminUpdateProfile = () => {
    ...
    useEffect(() => {
        const fetchProfileData = async () => {
            try {
                const response = await axios.get(`http://localhost:7070/admin/getAdmin/${auth.currentUser.uid}`);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await axios.put(`http://localhost:7070/admin/updateAdmin/${auth.currentUser.uid}`, updatedProfileData);
