...
const AllTournamentsTable = () => {
    ...
    const fetchEligibleUpcomingTournaments = async () => {
        try {
            const response = await axios.get(`http://matchup-load-balancer-1173773587.ap-southeast-1.elb.amazonaws.com:8080/api/tournaments/eligible/${auth.currentUser.uid}`);
