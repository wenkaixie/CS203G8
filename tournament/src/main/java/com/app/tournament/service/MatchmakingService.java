...
public class MatchmakingService {
    ...
    public List<RoundMatchDTO> createPairingsForRound(String tournamentId, int roundNumber) throws Exception {
        try {
            // Fetch Users for the tournament
            List<UserInfo> users = getUsersByTournamentId(tournamentId);

            // Initialize pairings list and paired Users set
            List<RoundMatchDTO> pairings = new ArrayList<>();
            Set<String> pairedUsers = new HashSet<>();

            // For the first round, pair by Elo
            if (roundNumber == 1) {
                // Sort Users by Elo rating
                Collections.sort(users, Comparator.comparing(UserInfo::getElo).reversed());

                // Pair top half against the bottom half
                for (int i = 0; i < users.size() / 2; i++) {
                    UserInfo User1 = users.get(i); // Top half User
                    UserInfo User2 = users.get(i + users.size() / 2); // Bottom half User
                ...
                Collections.sort(users, Comparator.comparing(UserInfo::getScore).reversed());
            ...
            for (RoundMatchDTO match : pairings) {
    ...
    private boolean assignWhiteBlack(UserInfo User1, UserInfo User2) {
        int whiteGamesPlayer1 = User1.getWhiteGamesCount();
        int whiteGamesPlayer2 = User2.getWhiteGamesCount();

        // If one player has played white significantly more, assign the other player
        // white
        if (whiteGamesPlayer1 > whiteGamesPlayer2) {
            return false; // Player 2 gets white
        } else if (whiteGamesPlayer2 > whiteGamesPlayer1) {
            return true; // Player 1 gets white
        }
        return Math.random() < 0.5; // Randomly assign white/black
    }

    // Helper method to create a match
    private RoundMatchDTO createMatch(UserInfo User1, UserInfo User2, boolean User1IsWhite, int roundNumber) {
        ...
        match.setRid(String.valueOf(roundNumber));
    ...
    private void assignBye(UserInfo User, int roundNumber) {
        System.out.println("Assigning a bye to User: " + User.getUid());
    ...
    private List<UserInfo> getUsersByTournamentId(String tournamentId) throws ExecutionException, InterruptedException {
        ApiFuture<QuerySnapshot> future = firestore.collection("Tournaments")
                .document(tournamentId)
                .collection("UserInfo")
                .get();
        return future.get().toObjects(UserInfo.class);
    }
}
